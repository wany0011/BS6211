"""
Revision history
01 - skip those with error (raise exception)
...
def prune(inds):
	...
	for pt in end_pts:
		...
		while True:
			...
			prev_pt = curr_pt
			curr_pt = next_pt
		if len(branch) > 50: 	# skip main branch !!! choose an appropriate number
			continue
...
class MainCurve(object):
	...
	def __init__(self, mask, op_name):
		...
		if loop >= 100:
			...
			err_string = 'Warning: Looped 100 times, remaining branches may mess up ordering.\n'
			with open(Config.log_file, 'a') as f:
				f.write(op_name + " " + err_string)
			cv2.imwrite(op_name, bgr_mask)
		...
		if inds[start_pt][1] > inds[end_pt][1]:
			#with open('/home/liuwei/Angio/RCA_annotated/png/Warning.log', 'a') as log:
			with open('./Temp/Warning.log', 'a') as log:
